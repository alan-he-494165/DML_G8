"""
Train trinary XGBoost variants using chronological splits only.

This script does not modify the existing xgboost/ benchmark outputs.
All artifacts are written under xgboost_trinary_chrono/.
"""
...
OUT_DIR = ROOT / "xgboost_trinary_chrono"
MODELS_DIR = OUT_DIR / "models"
PLOTS_DIR = OUT_DIR / "plots"
BENCH_PATH = OUT_DIR / "chrono_variant_benchmarks.json"
...
def save_benchmark_plot(results: dict, order: list[str]):
    ...
    ax.set_title("Chronological Trinary Variant Benchmark Comparison (Test Set)")
    ax.set_ylabel("Score")
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(PLOTS_DIR / "chrono_variant_benchmarks.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    variants = [
        ("original_chrono", base_X_df, base_X, conf, params_original, False),
        ("class_weight_chrono", base_X_df, base_X, w_class, params_original, False),
        ("threshold_features_chrono", eng_X_df, eng_X, conf, params_original, True),
        ("class_weight_threshold_chrono", base_X_df, base_X, w_class, params_original, True),
        ("tuned_hparams_chrono", base_X_df, base_X, conf, params_tuned, False),
        ("threshold_tuned_hparams_chrono", base_X_df, base_X, conf, params_tuned, True),
    ]
