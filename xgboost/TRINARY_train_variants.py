...
- method1_4_6_class_weight_threshold: class weighting + threshold tuning
- method6_chrono_threshold_original_hparams: original hyperparameters + chronological split + threshold tuning
- method7_8_hparams_chrono: tuned hyperparams + chronological split
- method6_7_8_threshold_hparams_chrono: threshold tuning + chronological tuned hyperparams
"""
...
README_TRINARY_PATH = ROOT / "xgboost" / "TRINARY_README_VARIANTS.md"
...
def build_readme(results: dict):
    ...
    lines.append("- `xgboost/models/method1_4_6_class_weight_threshold.pkl`")
    lines.append("- `xgboost/models/method6_chrono_threshold_original_hparams.pkl`")
    lines.append("- `xgboost/models/method7_8_hparams_chrono.pkl`")
    lines.append("- `xgboost/models/method6_7_8_threshold_hparams_chrono.pkl`")
    lines.append("")
    ...
    lines.append("- `method1_4_6_class_weight_threshold`: inverse-frequency class weighting plus validation-tuned class thresholds.")
    lines.append("- `method6_chrono_threshold_original_hparams`: original hyperparameters + chronological split + validation-tuned class thresholds.")
    lines.append("- `method7_8_hparams_chrono`: tuned hyperparameters + chronological split (harder, more realistic).")
    lines.append("- `method6_7_8_threshold_hparams_chrono`: validation-tuned thresholds + chronological split + tuned hyperparameters.")
    lines.append("")
    ...
    README_TRINARY_PATH.write_text("\n".join(lines).replace("README_VARIANTS", "TRINARY_README_VARIANTS"), encoding="utf-8")


def save_benchmark_plot(results: dict):
    order = [
        "original",
        "method1_class_weight",
        "method4_6_threshold_features",
    ...
    ax.set_title("Trinary Variant Benchmark Comparison (Test Set)")
    ax.set_ylabel("Score")
    ...
    out = BENCH_DIR / "trinary_variant_benchmarks.png"
...
def main():
    ...
    split_m6co = SplitData(
        *chronological_split(df, base_X, y, conf),
        feature_cols=list(base_X_df.columns),
        split_type="chronological",
    )
    split_m678 = SplitData(
        *chronological_split(df, base_X, y, conf),
        feature_cols=list(base_X_df.columns),
        split_type="chronological",
    )
    ...
    print("Training method1_4_6_class_weight_threshold...")
    tmp_model = xgb.XGBClassifier(**params_m1)
    tmp_model.fit(split_m146.X_train, split_m146.y_train, sample_weight=split_m146.w_train, verbose=False)
    val_proba = tmp_model.predict_proba(split_m146.X_val)
    best_th_m146 = tune_thresholds(split_m146.y_val, val_proba, split_m146.w_val)
    res_m146 = train_variant("method1_4_6_class_weight_threshold", split_m146, params_m1, thresholds=best_th_m146)

    print("Training method6_chrono_threshold_original_hparams...")
    tmp_model = xgb.XGBClassifier(**params_original)
    tmp_model.fit(split_m6co.X_train, split_m6co.y_train, sample_weight=split_m6co.w_train, verbose=False)
    val_proba = tmp_model.predict_proba(split_m6co.X_val)
    best_th_m6co = tune_thresholds(split_m6co.y_val, val_proba, split_m6co.w_val)
    res_m6co = train_variant("method6_chrono_threshold_original_hparams", split_m6co, params_original, thresholds=best_th_m6co)

    print("Training method7_8_hparams_chrono...")
    res_m78 = train_variant("method7_8_hparams_chrono", split_m78, params_m78)

    print("Training method6_7_8_threshold_hparams_chrono...")
    tmp_model = xgb.XGBClassifier(**params_m78)
    tmp_model.fit(split_m678.X_train, split_m678.y_train, sample_weight=split_m678.w_train, verbose=False)
    val_proba = tmp_model.predict_proba(split_m678.X_val)
    best_th_m678 = tune_thresholds(split_m678.y_val, val_proba, split_m678.w_val)
    res_m678 = train_variant(
        "method6_7_8_threshold_hparams_chrono",
        split_m678,
        params_m78,
        thresholds=best_th_m678,
    )
    ...
    results = {
        ...
        "method1_4_6_class_weight_threshold": res_m146,
        "method6_chrono_threshold_original_hparams": res_m6co,
        "method7_8_hparams_chrono": res_m78,
        "method6_7_8_threshold_hparams_chrono": res_m678,
    }

    BENCH_DIR.mkdir(parents=True, exist_ok=True)
    with open(BENCH_DIR / "trinary_variant_benchmarks.json", "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    with open(BENCH_DIR / "TRINARY_benchmark_variants.json", "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)

    build_readme(results)
    save_benchmark_plot(results)
    print("Saved models, benchmarks JSON, benchmark PNG, and README_VARIANTS.md")
